...
pg.connect(config.DATABASE_URL, function (err, client) {
	...
	/*client.query("DROP TABLE IF EXISTS users", function(err, result) {
		...
		client.query('CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY, name VARCHAR(30), alias VARCHAR(20), password VARCHAR(300), email VARCHAR(30), interests VARCHAR(200), latitude VARCHAR(30), longitude VARCHAR(30))', function(err, result) {
			if (err) {
				console.log(err);
				throw err;
			}
