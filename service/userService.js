...
UserService.prototype.create = function(data, next) {
    ...
    client.query("SELECT * FROM users WHERE alias = ($1)", [data.username], function (err, result) {
      ...
      var user = [ data.name || null, data.username || null, encryptedPassword, data.email || null, data.sex || null, data.age || null, data.latitude || null, data.longitude || null, photo_profile_base64 ];
      client.query("INSERT INTO users(name, alias, password, email, sex, age, latitude, longitude, photo_profile) values($1, $2, $3, $4, $5, $6, $7, $8, $9)", user, function (err, result) {
      	if (err) {
      		console.log(err);
      		next({message: ""}, null);
  ...
  pg.connect(config.DATABASE_URL, function (err, client, done) {
    client.query("SELECT * FROM users WHERE alias = ($1)", [alias], function (err, result) {
      if (result.rowCount) {
      	...
      	var query = client.query("SELECT * FROM userInterests"); //AS ui WHERE ui.userid <> " + result.rows[0].id.toString() + " AND ui.interestid IN (SELECT interestid FROM userInterests AS myui WHERE myui.userid = " + result.rows[0].id.toString());
    		// Stream results back one row at a time
		    query.on('row', function (row, result) {
		      result.addRow(row);
		    });
		    ...
		    query.on('end', function (result) {
		    	...
		    	_.each(result.rows, function(user) {
		    		client.query("SELECT * FROM users WHERE id = ($1)", [user.userid], function (err, result) {
		    			if (result.rowCount) {
		    				jsonObject.users.push(result.rows[0]);
		    			}
		    			count--;
		    			if (count == 0) {
